...
def select(Input_parameter):
    if Input_parameter == 'All Sites':
        ...
        new_df = spacex_df[spacex_df["Launch Site"] == Input_parameter]["class"].value_counts().to_frame()
        new_df["name"] = ["Failure", "Success"]
        fig = px.pie(new_df, values='class', names='name', title='Total Success Launches for ' + Input_parameter)
    return fig
